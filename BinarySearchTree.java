public class BinarySearchTree<Key extends Comparable<Key>, Value> {
    ...
    private Node<Key, Value> put(Node<Key, Value> n, Key key, Value val) {
        ...
        int random = key.compareTo(n.getKey());
        if      (random < 0) n.setLeft(put(n.getLeft(),  key, val));
        else if (random > 0) n.setRight(put(n.getRight(), key, val));
        else              n.setValue(val);
    ...
    public boolean contains(Key key) {
        if (key == null) throw new IllegalArgumentException("not found(null)");
        return get(key) != null;
    }
